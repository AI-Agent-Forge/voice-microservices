...
import { usePracticeStore, getPhonemeStatsFromSessions } from '../stores/practice'
import { Mic, TrendingUp, Clock, Award, Play, History, AlertTriangle, CheckCircle, Zap } from 'lucide-react'
...
export default function Dashboard() {
  ...
  // Calculate phoneme statistics from all sessions
  const phonemeStats = useMemo(() => getPhonemeStatsFromSessions(sessions), [sessions])

  // Get most problematic and best phonemes
  const sortedPhonemes = useMemo(() => {
    const entries = Object.entries(phonemeStats)
      .filter(([_, stats]) => stats.total >= 2) // Need at least 2 occurrences
      .map(([phoneme, stats]) => ({
        phoneme,
        ...stats,
        errorRate: stats.total > 0 ? (stats.errors / stats.total) * 100 : 0,
        accuracy: stats.total > 0 ? ((stats.total - stats.errors) / stats.total) * 100 : 100
      }))

    const problematic = [...entries].sort((a, b) => b.errorRate - a.errorRate).slice(0, 3)
    const best = [...entries].sort((a, b) => a.errorRate - b.errorRate).slice(0, 3)

    return { problematic, best }
  }, [phonemeStats])
  ...
  return (
    ...
    <div className="px-6 space-y-8 max-w-7xl mx-auto pb-12">
      ...
      {/* Phoneme Statistics - Shows data from Phoneme-Diff service */}
      {(sortedPhonemes.problematic.length > 0 || sortedPhonemes.best.length > 0) && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-white">Phoneme Analysis</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Areas to Improve */}
            {sortedPhonemes.problematic.length > 0 && (
              <div className="bg-slate-900/50 rounded-xl p-6 border border-orange-500/20">
                <div className="flex items-center space-x-2 mb-4">
                  <AlertTriangle className="w-5 h-5 text-orange-400" />
                  <h4 className="text-white font-medium">Focus Areas</h4>
                </div>
                <p className="text-slate-400 text-sm mb-4">Phonemes that need more practice based on your session history</p>
                <div className="space-y-3">
                  {sortedPhonemes.problematic.map((item, i) => (
                    <div key={i} className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <span className="font-mono text-lg text-orange-400 bg-orange-500/10 px-2 py-1 rounded">
                          {item.phoneme}
                        </span>
                        <span className="text-slate-400 text-sm">
                          {item.errors}/{item.total} attempts
                        </span>
                      </div>
            ...
            {sortedPhonemes.best.length > 0 && (
              <div className="bg-slate-900/50 rounded-xl p-6 border border-green-500/20">
                ...
                <p className="text-slate-400 text-sm mb-4">Phonemes you pronounce consistently well</p>
                <div className="space-y-3">
                  {sortedPhonemes.best.map((item, i) => (
                    <div key={i} className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <span className="font-mono text-lg text-green-400 bg-green-500/10 px-2 py-1 rounded">
                          {item.phoneme}
                        </span>
                        <span className="text-slate-400 text-sm">
                          {item.total - item.errors}/{item.total} correct
                        </span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <div className="w-20 h-2 bg-slate-700 rounded-full overflow-hidden">
